...
class Yks_Sortable_Columns_Fields {
	...
	public function __construct( $the_column ) {
		...
		//Sortable
		if ( false !== $this->_column['sortable_column'] ) {
		...
		//admin ajax function
		add_action( 'wp_ajax_yks_star_action', array( $this, 'get_admin_ajax' ) );
	}
...
if ( is_admin() ) {
	// Vars.
	$yks_mboxes_check = apply_filters( 'yks_mboxes', array() );
	// Function
	...
	Execute_Yks_Sortable_Columns_Fields( $yks_mboxes_check );
}