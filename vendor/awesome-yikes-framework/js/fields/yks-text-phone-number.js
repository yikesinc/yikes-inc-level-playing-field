jQuery( function( $ ) {
	...
	$( '.yks_phone_number_container' ).sortable({
		cancel: 'input, textarea, button, select, option, a',
	});

	// If delete button is clicked, remove the associated field <li>, update the field counter
	$( 'body' ).on( 'click', '.yks_phone_number_delete', function() {
		yks_starter_repeating_fields_delete( this, '.yks_phone_number' );
	});

	// If add button is clicked, add another field
	$( '.yks_phone_number_add' ).click( function() {
		yks_starter_repeating_fields_duplicate_multiple( this, '.yks_phone_number' );
	});
