...
if ( $field_repeating === true ) {
	...
	wp_enqueue_script( 'yks-text-url', YKS_MBOX_URL . 'js/fields/min/yks-text-url.min.js', array( 'jquery' ) );

	// Container for repeating fields
	$field_html .= '<ul class="yks_txt_url_container" id="container_' . $field_id . '">';
	...
	foreach ( $field_values as $value ) {
		...
		$field_html .= '<li class="yks_txt_url_field ui-state-default"><span data-code="f156" class="dashicons dashicons-sort" ></span>';
		$field_html .= '<input type="text" class="yks_txt_url yks_repeating_input_fields_width" name="' . esc_attr( $field_id ) . '[' . $field_counter . ']" id="' . esc_attr( $field_id ) . '_' . $field_counter . '" value="' . htmlspecialchars( $value ) . '" />';
		$field_html .= '<span class="yks_txt_url_delete dashicons dashicons-dismiss" ' . $style . '></span>';
		$field_html .= '</li>';
	}
	...
	// Add field button
	$field_html .= '<div class="yks_txt_url_add_container">';
	$field_html .= '<span class="yks_txt_url_add button button-primary" data-field-count="' . esc_attr( $field_counter ) . '">' . $repeat_btn_text . '</span>';
	$field_html .= '</div>';
} else {
	...
	$field_html .= '<input type="text" class="yks_txt_url" name="' . esc_attr( $field_id ) . '" id="' . esc_attr( $field_id ) . '" value="' . htmlspecialchars( $value ) . '" />';
}
