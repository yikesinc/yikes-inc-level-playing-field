...
/**
*
* Radio buttons for the different terms within the taxonomy
*
* Nothing is 'saved' for this field - if a taxonomy is checked off we simply relate the $post and the $taxonomy
*
* So we need to check the current post's terms (get_the_terms) to determine whether a radio button should be pre-checked
*
*/
...
if ( ! is_wp_error( $terms ) ) {
	if ( ! empty( $terms ) ) {
		$field_html .= '<ul class="yks_taxonomy_container">';
		foreach ( $terms as $term ) {
			...
			$field_html .= '<li>';
			$field_html .= '<input type="radio" class="yks_taxonomy_radio" name="' . esc_attr( $field_id ) . '" id="' . esc_attr( $field_id ) . '_' . $field_counter . '" value="' . esc_attr( $term_slug ) . '"' . $checked . '>';
