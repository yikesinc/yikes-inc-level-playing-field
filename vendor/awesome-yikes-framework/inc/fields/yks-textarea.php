...
if ( $field_repeating === true ) {
	...
	wp_enqueue_script( 'yks-textarea', YKS_MBOX_URL . 'js/fields/min/yks-textarea.min.js', array( 'jquery' ) );

	// Container for repeating fields
	$field_html .= '<ul class="yks_textarea_container" id="container_' . $field_id . '">';
	...
	foreach ( $field_values as $value ) {
		...
		$field_html .= '<textarea class="yks_textarea yks_repeating_input_fields_width" name="' . esc_attr( $field_id ) . '[' . $field_counter . ']" id="' . esc_attr( $field_id ) . '_' . $field_counter . '" cols="60" rows="10">' . $value . '</textarea>';
		$field_html .= '<span class="yks_textarea_delete dashicons dashicons-dismiss" ' . $style . '></span>';
		$field_html .= '</li>';
	}
	...
	$field_html .= '<span class="yks_textarea_add button button-primary" data-field-count="' . esc_attr( $field_counter ) . '">' . $repeat_btn_text . '</span>';
	$field_html .= '</div>';
} else {
