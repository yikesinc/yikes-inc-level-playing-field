...
/**
 * YIKES_Level_Playing_Field_Breadcrumbs class.
 *
 * @class 		YIKES_Level_Playing_Field_Breadcrumbs
 * @version		1.0.0
 * @author 		YIKES, Inc.
 */
class YIKES_Level_Playing_Field_Breadcrumbs {
	...
	public function get_breadcrumb() {
		return apply_filters( 'yikes_lpf_get_breadcrumb', $this->crumbs, $this );
	}
	...
	private function add_crumbs_404() {
		$this->add_crumb( __( 'Error 404', 'yikes-inc-level-playing-field' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'product' === get_post_type( $post ) ) {
			...
			if ( $terms = wc_get_product_terms( $post->ID, 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
				$main_term = apply_filters( 'yikes_lpfbreadcrumb_main_term', $terms[0], $terms );
	...
	private function add_crumbs_product_tag() {
		...
		$this->prepend_shop_page();
		$this->add_crumb( sprintf( __( 'Products tagged &ldquo;%s&rdquo;', 'yikes-inc-level-playing-field' ), $current_term->name ) );
	}
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( __( 'Posts tagged &ldquo;%s&rdquo;', 'yikes-inc-level-playing-field' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( __( 'Author: %s', 'yikes-inc-level-playing-field' ), $userdata->display_name ) );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( __( 'Search results for &ldquo;%s&rdquo;', 'yikes-inc-level-playing-field' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( __( 'Page %d', 'yikes-inc-level-playing-field' ), get_query_var( 'paged' ) ) );
		}
	}
}
